...
int main() {
  ...
  while(!q2.empty()) {
    ...
    for(auto e : graph[v].rev_rels) {
      graph[e].min_rels_time = max(graph[e].min_rels_time, graph[v].min_rels_time + graph[v].time);
      q2.push(e);
    }
  }
  // print_graph(n);
